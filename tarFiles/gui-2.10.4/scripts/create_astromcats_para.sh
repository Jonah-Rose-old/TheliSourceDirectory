...
do
  ...
  cat ${TEMPDIR}/astromimages_$$ |\
  {
    ...
    do
	  ...
	  # remove objects that lie outside the actual image area  (a weird sextractor thing)
	  ${P_LDACFILTER} -i /$1/$2/cat/${BASE}.cat0 \
	      -o /$1/$2/cat/${BASE}.cat1 \
	      -t LDAC_OBJECTS -c "(((((FLAGS<=${V_SEXCATFLAG})AND(FWHM_IMAGE>${V_SEXCATMINFWHM}))AND(FLUX_RADIUS>${minfluxrad}))AND(NIMAFLAGS_ISO<=2))AND(FLAGS!=1));"
      fi
      ...
      # first, check how many usable objects are there
      ${P_LDACTOASC} -i /$1/$2/cat/${BASE}.cat2 -t OBJECTS \
          -k Flag -b | ${P_GAWK} '($1==0)' > \
	  ${TEMPDIR}/checkobjcounts_$$
      ngood=`wc -l ${TEMPDIR}/checkobjcounts_$$ | ${P_GAWK} '{print $1}'`
...
#	  ${P_GAWK} '($1==0)' | wc | ${P_GAWK} '{print $1}'`
	  ...
	  HFD_WORLD=-1
	  HFD_IMAGE=0
	  # we still have to create ${BASE}.cat
	  ${P_LDACADDKEY} -i /$1/$2/cat/${BASE}.cat2 -o /$1/$2/cat/${BASE}.cat3 -t FIELDS \
	      -k FWHM_IMAGE ${FWHM_IMAGE} FLOAT "Median FWHM_IMAGE of all sources / UNDETERMINED"
	  ${P_LDACADDKEY} -i /$1/$2/cat/${BASE}.cat3 -o /$1/$2/cat/${BASE}.cat4 -t FIELDS \
	      -k FWHM_WORLD ${FWHM_WORLD} FLOAT "Median FWHM_WORLD of all sources / UNDETERMINED"
	  ${P_LDACADDKEY} -i /$1/$2/cat/${BASE}.cat4 -o /$1/$2/cat/${BASE}.cat5 -t FIELDS \
	      -k HFD_IMAGE ${HFD_IMAGE} FLOAT "Median HFD_IMAGE of all sources / UNDETERMINED"
	  ${P_LDACADDKEY} -i /$1/$2/cat/${BASE}.cat5 -o /$1/$2/cat/${BASE}.cat -t FIELDS \
	      -k HFD_WORLD ${HFD_WORLD} FLOAT "Median HFD_WORLD of all sources / UNDETERMINED"
      fi

      # make the astrometry.net catalogs
      ${P_LDACTOASC} -i /$1/$2/cat/${BASE}.cat -t OBJECTS -k XWIN_IMAGE YWIN_IMAGE MAG_AUTO -b \
	    > ${TEMPDIR}/${BASE}_an1.cat
      ${P_ASCTOLDAC} -a ${TEMPDIR}/${BASE}_an1.cat -o ${TEMPDIR}/${BASE}.anet -t OBJECTS \
	  -c ${DATACONF}/anet.asctoldac_img.conf
      # we have to delete the FIELDS table as otherwise solve-field does not find
      # the MAG key in the OBJECTS table (bug in a.net)
      ${P_LDACDELTAB} -i ${TEMPDIR}/${BASE}.anet -o /$1/$2/cat/${BASE}.anet -t FIELDS
      rm ${TEMPDIR}/${BASE}.anet
	  ...
	  -k ALPHA_J2000 DELTA_J2000 AWIN_WORLD BWIN_WORLD THETAWIN_IMAGE -b \
	...
	mv /$1/$2/${BASE}*.fits /$1/$2/lownum
      ...
      \rm /$1/$2/cat/${BASE}.cat[0,2,3,4,5] ${TEMPDIR}/${BASE}_an1.cat
    done
  }
done
