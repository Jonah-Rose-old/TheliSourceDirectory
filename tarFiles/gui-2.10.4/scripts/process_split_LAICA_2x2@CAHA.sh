...
# the data is already split, so just loop over all four detectors
# and update the FITS header

CHIP=1
while [ "${CHIP}" -le "${NCHIPS}" ]
do

  FILES=`ls -1l $1/$2/*${CHIP}.fits | ${P_GAWK} '($1 ~ /^-/) {print $NF}'`
  ...
  for FILE in ${FILES}
    ...
    FILTER=`get_key ${FILE} "HIERARCH CAHA INS FILT1 NAME" | ${P_CLEANSTRING}`
    ...
    ${P_REPLACEKEY} ${BASE}_${CHIP}.fits "DATE-OBS= ${tmp}" DUMMY1
    ${P_REPLACEKEY} ${BASE}_${CHIP}.fits "IMAGEID = ${CHIP}" IMAGEID
    ${P_REPLACEKEY} ${BASE}_${CHIP}.fits "MJD-OBS = ${MJD}" DUMMY2

    mv ${FILE} ORIGINALS/${BASE}${SUFFIX}

    # cancel the renaming of mefsplit
    mv ${BASE}_${CHIP}.fits ${FILE}
