...
version = string.split(string.replace(string.split(sys.version)[0],
                                      '+', ''), ".")
...
for i in range(len(tableindices)):
    ...
    for k in range(len(xi)):
        ...
        # deal with the Ra=0 pole:
        if RaCorr[k] < 0.0:
            RaCorr[k] = RaCorr[k] + 360.0
        if RaCorr[k] > 360.0:
            RaCorr[k] = RaCorr[k] - 360.0
    ...
    # objects table:
    RaCorrColumn = fits.Column(name = options.resultposkeys[0],
                               format = '1D', unit = 'deg',
                               array = RaCorr)
    
    DecCorrColumn = fits.Column(name = options.resultposkeys[1],
                                format = '1D', unit = 'deg',
                                array = DecCorr)
    ...
    newObjectTableHDU =  fits.new_table(newObjectColumns,
