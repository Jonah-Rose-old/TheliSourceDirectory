...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
        ...
        The original version of this software, called LMFIT, was
        written in FORTRAN as part of the MINPACK-1 package by XXX.

        Craig Markwardt converted the FORTRAN code to IDL.  The
        information for the IDL version is: Craig B. Markwardt,
        NASA/GSFC Code 662, Greenbelt, MD 20770
        craigm@lheamail.gsfc.nasa.gov UPDATED VERSIONs can be found on
        my WEB PAGE: http://cow.physics.wisc.edu/~craigm/idl/idl.html

        Mark Rivers created this Python version from Craig's IDL
        version.  Mark Rivers, University of Chicago Building
        434A, Argonne National Laboratory 9700 South Cass
        Avenue, Argonne, IL 60439 rivers@cars.uchicago.edu
        ...
        implementation is drawn from MINPACK-1 (see NETLIB), and is
        much faster and more accurate than the version provided in the
        Scientific Python package in
        Scientific.Functions.LeastSquares.  This version allows upper
        and lower bounding constraints to be placed on each parameter,
        or the parameter can be held fixed.
        ...
        values as specified above.  The function should return the
        weighted deviations between the model and the data.  It should
        also return a status flag and an optional partial derivative
        array.  For applications which use finite-difference
        derivatives -- the default -- the user function should be
        ...
        Derivatives should be returned in the PDERIV array. PDERIV
        should be an m x n array, where m is the number of data points
        and n is the number of parameters.  dp[i,j] is the derivative
        at the ith point with respect to the jth parameter.
        ...
        PARINFO should be a list of dictionaries, one list entry for
        each parameter.  Each parameter is associated with one element
        of the array, in numerical order.  The dictionary can have the
        following keys (none are required, keys are case insensitive):
...
xall:
        An array of starting values for each of the parameters of the
...
fastnorm:
        Set this keyword to select a faster algorithm to compute
        sum-of-square values internally.  For systems with large
        numbers of data points, the standard algorithm can become
        prohibitively slow because it cannot be vectorized well.  By
        setting this keyword, MPFIT will run faster, but it will be
        more prone to floating point overflows and underflows.  Thus,
        setting this keyword may sacrifice some stability in the
        fitting process.  Default: clear (=0)
...
iterfunct:
        ...
        In implementation, iterfunct can perform updates to the
        terminal or graphical user interface, to provide feedback
        while the fit proceeds. If the fit is to be stopped for any
        reason, then iterfunct should return a a status value between
        -15 and -1.  Otherwise it should return None (e.g. no return
        statement) or 0.  In principle, iterfunct should probably not
        modify the parameter values, because it may interfere with the
...
.perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        ## Parameters can either be stored in parinfo, or x. x takes
        ...
        while(1):
            ...
            if (nprint > 0) and (iterfunct != None):
                if (((self.niter-1) % nprint) == 0):
                    ...
                    status = iterfunct(fcn, self.params,
                                       self.niter, self.fnorm**2,
                    ...
                    if (max(abs(xnew0-self.params)) > 0):
                        if (self.qanytied): self.params = \
                            self.tie(self.params, ptied)
            ...
            while(1):
                ...
                if (qanylim == 0) and (qminmax == 0):
                    ...
                    ## Respect the limits.  If a step were to go out
                    ## of bounds, then we should take a step in the
                    ## same direction but shorter distance.  The step
                    ## should take us right to the limit in that case.
                    alpha = 1.
                    ...
                    if (qanylim):
                        ...
                        dwa1 = abs(wa1) > machep
                        whl = (numpy.nonzero(((dwa1!=0.) & qllim) &
                              ((x + wa1) < llim)) )[0]
                    ...
                    ## Adjust the final output values.  If the step
                    ## put us exactly on a boundary, make sure it is
                    ## exact.
                    wh = (numpy.nonzero((qulim!=0.) &
                                        (wa2 >= ulim*(1-machep))))[0]
                    if (len(wh) > 0): numpy.put(wa2, wh, numpy.take(ulim, wh))
                    wh = (numpy.nonzero((qllim!=0.) &
                                        (wa2 <= llim*(1+machep))))[0]
    ...
    def defiter(self, fcn, x, iter, fnorm=None, functkw=None,
        ...
        nprint = len(x)
        print "Iter ", ('%6i' % iter), \
              "   CHI-SQUARE = ",('%.10g' % fnorm)," DOF = ", ('%i' % dof)
    ...
    ## Procedure to parse the parameter values in PARINFO, which is a
    ...
    def call(self, fcn, x, functkw, fjac=None):
        ...
        if (fjac == None):
            ...
            if (self.damp > 0):
                ...
                ## residuals with their hyperbolic tangent.  Thus
                ## residuals larger than DAMP are essentially clipped.
    ...
    def enorm(self, vec):
        ...
        ## NOTE: it turns out that, for systems that have a lot of
        ## data points, this routine is a big computing bottleneck.
        ## The extended computations that need to be done cannot be
        ## effectively vectorized.  The introduction of the FASTNORM
        ## configuration parameter allows the user to select a faster
        ## routine, which is based on TOTAL() alone.
        ...
        if (self.fastnorm):
            ...
            ## robustness.  Need to do this because of the possibility
    ...
    def fdjac2(self, fcn, x, fvec, step=None, ulimited=None,
        ...
        if (autoderivative == 0):
            ...
            ## This definition is c1onsistent with CURVEFIT Sign error
            ## found (thanks Jesus Fernandez
        ...
        if len(ulimited) > 0 and len(ulimit) > 0:
            # The mask modification below makes problems
            # if the dimensions of mask, ulimited and ulimit
            # are not the same (TE) 
            mask = mask[0:(len(ulimited))]
        ...
        for j in range(minmn):
            if (pivot != 0):
                ...
                if ct > 0:
                    ...
                    ## Exchange rows via the pivot only.  Avoid
                    ## actually exchanging the rows, in case there is
                    ## lots of memory transfer.  The exchange occurs
                    ## later, within the body of MPFIT, after the
                    ## extraneous columns of the matrix have been
                    ## shed.
                    if kmax != j:
                        temp = ipvt[j]
                        ipvt[j] = ipvt[kmax]
