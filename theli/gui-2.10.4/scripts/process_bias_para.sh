...
# the script processes a set of BIAS frames.
...
# $1: master directory
# $2: subdirectory with the BIAS images
# $3: chips to work on
...
# exists already. if yes, then delete it.
  ...
  if [ "${numexp}" -lt 3 ]; then
      echo PREPROCESS_FAILURE: Need at least three images of chip ${CHIP} to create master bias
      continue
  fi
...
#	  -STATSSEC ${statminsection}
  ...
  # and combine them
      ls -1 $1/$2/*_${CHIP}OC.fits > ${TEMPDIR}/bias_images_$$
      ${P_IMCOMBFLAT_IMCAT} -i ${TEMPDIR}/bias_images_$$\
	  -o ${RESULTDIR[${CHIP}]}/$2_${CHIP}.fits \
	  -e ${V_CAL_BIASNLOW} ${V_CAL_BIASNHIGH} 
  fi
  ...
  # overscan correct and trim BIAS frames
	  ...
	  -COMBINE_REJECT ${V_CAL_BIASNLOW},${V_CAL_BIASNHIGH} \
	  -STATSSEC ${xmin},${xmax},${ymin},${ymax}
#	  -STATSSEC ${statminsection} \
  fi
