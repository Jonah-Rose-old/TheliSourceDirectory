...
do
  ...
  # Paste them into a single image
  ${P_PASTELASCUMBRES} -i ${BASE} -o FL03.${FILTER}.$DATEOBS.fits -n $INSTRUMENT
  rm ${BASE}_sl0[1-4].fits 
  mv ${FILE} ORIGINALS
done

# now do the standard splitting
FILES=`ls -1l $1/$2/FL03*.fits | ${P_GAWK} '($1 ~ /^-/) {print $NF}'`
