...
do
  ...
  FILTER=`get_key ${FILE} "FILTRE" | ${P_CLEANSTRING}`
  if [ "${FILTER}" = "KEY_N/A" ]; then
  ...
  while [ "${k}" -le "${NCHIPS}" ]
  do
    tmp=`get_key ${FILE} "PHI"`
    ${P_REPLACEKEY} ${BASE}_${k}.fits "CROTA   = ${tmp}" DUMMY1
    tmp=`get_key ${FILE} "DATE-OBS"`
    ${P_REPLACEKEY} ${BASE}_${k}.fits "DATE-OBS= ${tmp}" DUMMY2
    tmp=`get_key ${FILE} "FAFLTNM"`
    ${P_REPLACEKEY} ${BASE}_${k}.fits "FAFLTNM = ${tmp}" DUMMY3
    tmp=`get_key ${FILE} "FBFLTNM"`
    ${P_REPLACEKEY} ${BASE}_${k}.fits "FBFLTNM = ${tmp}" DUMMY4
    ${P_REPLACEKEY} ${BASE}_${k}.fits "MJD-OBS = ${MJD}" DUMMY5
    k=$(( $k + 1 ))
  done
