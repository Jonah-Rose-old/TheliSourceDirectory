...
# splits GOODMAN_2x2@SOAR images
...
do
  ...
  FILT1=`get_key ${FILE} "FILTER" | ${P_CLEANSTRING}`
  ...
  do
    ...
    ${P_REPLACEKEY} ${BASE}_${k}.fits "FILENAME= ${FILE}" DUMMY3
    mv ${BASE}_${k}.fits GOODMAN.${tmp}_${k}.fits
    k=$(( $k + 1 ))
  done
