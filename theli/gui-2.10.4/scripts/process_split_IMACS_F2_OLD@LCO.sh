...
do
  ...
  for FILE in ${FILES}
    do
      ...
      ${P_REPLACEKEY} ${BASE}_${k}.fits "CRPIX1  = ${REFPIXX[$NUMBER]}" CRPIX1
      ${P_REPLACEKEY} ${BASE}_${k}.fits "CRPIX2  = ${REFPIXY[$NUMBER]}" CRPIX2
      
      date=`get_key ${FILE} DATE-OBS`
      timeobs=`get_key ${FILE} UT-TIME`
      dateobs=${date}T${timeobs}
      
      # rename the images to something meaningful
      mv ${BASE}_${k}.fits IMACS.${dateobs}_${k}.fits
      ${P_REPLACEKEY} IMACS.${dateobs}_${k}.fits "ORIGNAME= ${BASE}c${NUMBER}${SUFFIX}" DUMMY4
