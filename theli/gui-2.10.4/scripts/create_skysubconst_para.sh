...
if [ "${V_CSKY_XMIN}" != "" ] && [ "${V_CSKY_XMAX}" != "" ] && \
   [ "${V_CSKY_YMIN}" != "" ] && [ "${V_CSKY_YMAX}" != "" ]; then
    ...
    section="${V_CSKY_XMIN} ${V_CSKY_XMAX} ${V_CSKY_YMIN} ${V_CSKY_YMAX}"
fi
...
do
  ...
  get_statminsection
  if [ "${section}_A" = "_A" ]; then
      get_statminsection
      section=${statminsection_imstats}
  fi
  ...
  while read file
    do
      ...
      BASEred=`basename ${file} ${CHIP}$3.fits`
	  ...
	  sky=`${P_IMSTATS} -d 6 -s ${section} -t -1e8 1e8 ${RESULTDIR[${CHIP}]}/${BASE}"_skysub.fits" |\
               ${P_GAWK} '($1 != "#") {print $'"${pos}"'}'`
	 ...
	 [ "${V_CSKYMANUAL}" != "MANUAL" ] && [ "${area}" != "from_area" ]; then
	  ...
	  if [ -f $1/$2/.${BASEred}$4$3.sky ]; then
	      sky=`cat $1/$2/.${BASEred}$4$3.sky`
	      ...
	      ${P_SEX} $1/$2/${BASEred}$4$3.fits -c ${DATACONF}/image-objects.sex \
		  -DETECT_THRESH ${V_SKYSUBDETECTTHRESH} \
		  -DETECT_MINAREA ${V_SKYSUBDETECTMINAREA} \
		  -CHECKIMAGE_NAME ${RESULTDIR[${CHIP}]}/skysample.fits_$$ \
		  -WEIGHT_TYPE MAP_WEIGHT \
		  -WEIGHT_IMAGE /$1/WEIGHTS/${BASEred}$4$3.weight.fits \
		  -NTHREADS ${NUMTHREAD}

	      ${P_FITSPOLYGON} \
		  -i $1/$2/${BASEred}$4$3.fits \
	      ...
	      sky=`${P_IMSTATS} -d 6 -s ${section} -t -1e8 1e8 ${RESULTDIR[${CHIP}]}/skysample.sub.fits_$$ |\
                   ${P_GAWK} '($1 != "#") {print $'"${pos}"'}'`

	      echo ${sky} > $1/$2/.${BASEred}$4$3.sky
	 ...
	 [ "${V_CSKYMANUAL}" != "MANUAL" ]; then
	  ...
	  ls -1 /$1/$2/${BASEred}*$3."fits" > ${TEMPDIR}/sky2xy_imagelist_$$
	  ...
	  section_new=`${P_GAWK} '{print $2, $3, $4, $5}' ${TEMPDIR}/sky2xy_result_$$`
	  ...
	  if [ -f $1/$2/.${imagebase}.sky ]; then
		  ...
		  -t -1e8 1e8 -s ${section_new} | ${P_GAWK} '($1 != "#") {print $'"${pos}"'}'`	      
	  ...
	  ${P_IC} '%1 '"${sky}"' -' ${file} > ${RESULTDIR[${CHIP}]}/${BASE}.sub.fits
      fi
      ...
      skyexist=`get_key ${RESULTDIR[${CHIP}]}/${BASE}.sub.fits SKYVALUE`
      if [ "${skyexist}"_A = "_A" ]; then
	  NEXTDUMMY=`${P_DFITS} ${RESULTDIR[${CHIP}]}/${BASE}.sub.fits | grep DUMMY | \
	      ${P_GAWK} 'BEGIN {nrmin=1000+0} {
                        if (index($1,"DUMMY")==1) {
			   nr=substr($1,6)+0; if(nr<nrmin) {nrmin=nr}}} 
                        END {if(NR>0) {print nrmin}}'`
	  ${P_REPLACEKEY} ${RESULTDIR[${CHIP}]}/${BASE}.sub.fits \
