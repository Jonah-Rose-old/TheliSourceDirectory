...
# splits SuprimeCam200808_SDFRED@SUBARU images

# THIS SCRIPT DEVIATES VERY SIGNIFICANTLY FROM THE
# STANDARD SPLITTING PROCEDURE BY THELI

#
# We assume SDRED provides overscan-corrected, trimmed and flatfielded images!!!
#
...
for FILE in ${FILES}
do
  result=`${P_DFITS} ${FILE} | fitsort DATE-OBS UT-STR DET-ID | ${P_GAWK} '(NR==2) {print $2"T"$3, $4+1}'`
  dateobs=`echo ${result} | ${P_GAWK} '{print $1}'`
  chip=`echo ${result} | ${P_GAWK} '{print $2}'`
  ...
  # fix the gain differences between the chips (normalise to the highest gain, i.e. chip 9)
  # correction factors for CCDs 1-10:
  fac[1]=0.836 
  fac[2]=0.882 
  fac[3]=0.750 
  fac[4]=0.845 
  ...
  ${P_IC} '%1 '${fac[${chip}]}' *' ${BASE}_1.fits > SuprimeCam.${dateobs}_${chip}OFC.fits

  rm ${BASE}_1.fits

  ${P_REPLACEKEY} SuprimeCam.${dateobs}_${chip}OFC.fits "DATE-OBS= ${dateobs}" DUMMY1
  ${P_REPLACEKEY} SuprimeCam.${dateobs}_${chip}OFC.fits "MJD-OBS = ${MJD}" DUMMY2
done
