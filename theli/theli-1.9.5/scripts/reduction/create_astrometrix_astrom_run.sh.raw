...
#
# 21.12.2008:
# The perl interpreter is now called via a variable which is defined
# in progs.ini.
#
# 09.02.2009:
# I corrected a bug in the filtering of objects; saturated objects
# have flag value 4 instead of the 4th bit set! I now accept all
# objects having an internal SExtractor flag value smaller than
# 16 (none of the upper bits must be set!) and B_IMAGE>2.0!
...
do
  ...
  ${P_LDACFILTER} -i ${CAT} -t LDAC_OBJECTS -c "(FLAGS<16)AND(B_IMAGE>2.0);" \
                  -o ${BASE}.ldac
done
...
do
  ...
  BASE=`basename ${!j}`
  test -f nobjs_${!j} && rm nobjs_${BASE}
