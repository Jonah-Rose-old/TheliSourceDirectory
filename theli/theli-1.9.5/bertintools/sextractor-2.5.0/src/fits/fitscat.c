...
/* 23.05.2013:
   - I add THELI history information to newly created catalogues.
   - The function addhistoryto_cat now has a third argument indicating
     whether a timestamp should be added to the history by default. 
     Within THELI we do this ourselfs.

   10.02.2014:
   inclusion of t_iso8601.h to avoid a wrong, implicit inclusion of the
   function get_datetime_iso8601.
*/
...
/* Clean catalog and add the table to it */
  ...
  cat->ntab = 1;
/* add HISTORY information to the catalog */
  addhistoryto_cat(cat, " ", 0);
  sprintf(tmpstring, "sex_theli: %s", __PIPEVERS__);
  addhistoryto_cat(cat, tmpstring, 0);
  sprintf(tmpstring, "sex_theli: Catalog created at %s",
