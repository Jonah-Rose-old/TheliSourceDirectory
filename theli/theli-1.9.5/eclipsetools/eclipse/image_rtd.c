...
/*
	$Id: image_rtd.c,v 1.1 2003/09/04 15:44:34 terben Exp $
	$Author: terben $
...
static char * rtd_swapfilename(void)
{
	...
	tmpdir = xmemory_gettmpdir();
	if (tmpdir==NULL) {
		e_error("getting value for E_TMPDIR");
		return NULL ;
	}
	if (!strcmp(tmpdir, ".")) {
		...
		sprintf(swapname, "%s/eclipse-rtd.swp", tmpdir);
	}
	return swapname ;
}
