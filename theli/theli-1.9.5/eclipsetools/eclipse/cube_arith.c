...
/*
	$Id: cube_arith.c,v 1.2 2003/11/07 15:23:58 terben Exp $
	$Author: terben $
	$Date: 2003/11/07 15:23:58 $
	$Revision: 1.2 $
*/
		...
		double  threshhold)
{
	...
	if (cu==NULL || im==NULL) return -1 ;
	if ((cu->lx != im->lx) || (cu->ly != im->ly)) {
		e_error("incompatible size: cannot divide cube by image");
		return -1 ;
	}
	for (i=0 ; i<cu->np ; i++) {
		image_divthresh_local(cu->plane[i], im, threshhold);
	}
	return 0 ;
}
    ...
    pixelmap    *   badpix)
{
    ...
    /* In flatfield, replace by 0 the pixels whose value is <low and >high */
    corr_flat = NULL ;
    if (ff != NULL) {
        low = FLAT_LOW_THRESH ;
        high = FLAT_HIGH_THRESH ;
        if ((corr_flat = image_threshold(ff, low, high, 0, 0)) == NULL) {
           e_error("cannot threshold the flat") ;
           return -1 ;
        }
    }
    ...
    /* Free and return */
    if (ff != NULL) image_del(corr_flat) ;
    return 0 ;
}
