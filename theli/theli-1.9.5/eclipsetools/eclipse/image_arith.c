...
/*
	$Id: image_arith.c,v 1.2 2003/11/07 15:21:37 terben Exp $
	$Author: terben $
	$Date: 2003/11/07 15:21:37 $
	$Revision: 1.2 $
*/

/* 
   19.10.03:
   included new function image_divthresh_local (see the function for
   a description)

   07.11.03:
   included new functions image_bin and image_transpose
   (see the function for a description)
*/
...
/**
  @brief    Divide two images, store the result in the first image. 
            Do not perform the division and leave the original
            value intact if the absolut value of the
            divisor is lower than a given threshhold.
  @param    im1     first operand.
  @param    im2     second operand.
  @param    thresshold     divisor threshhold.
  @return   int 0 if Ok, -1 otherwise.
...
/**
  ...
  This ends up making all pixels in the image positive.
...
/*-------------------------------------------------------------------------*/
/**
  @brief    Perform a binning (demagnifying) of an image by a given factor
  @param    input       Input image.
  @param    bin         the scrunching factor
  @return   1 newly allocated image.
 
  The binning is done by calculating the mean of a bin x bin square right
  and up from the point under consideration.  
 */
/*--------------------------------------------------------------------------*/

image_t * image_bin(image_t *input, int bin)
{
  image_t *binned;
  int i, j, k, l;
  int nx ,ny;
  ...
  binned = image_new(nx, ny) ;

  for(i=0; i< ny; i++)
  {
    for(j=0; j< nx; j++)
    {
      px=input->data + (i*bin*input->lx + j*bin);
      sum = 0.0;
	...
	px += (input->lx - bin);
      ...
      sum /= (pixelvalue)(bin*bin);
      binned->data[i*binned->lx + j] = sum;
    }
  }
...
/*-------------------------------------------------------------------------*/
/**
  @brief    Performs a transposition of an image
  @param    input       Input image.
  @return   1 newly allocated image.
 
  The transposition exchanges x and y coordinates in a two-dimensional
  image
 */
/*--------------------------------------------------------------------------*/

image_t * image_transpose(image_t *input)
{
  image_t *transpose;
  int i, j;
  ...
  transpose = image_new(input->ly, input->lx) ;

  for(i=0; i < input->ly; i++)
  {
    for(j=0; j < input->lx; j++)
    {
      transpose->data[j*input->ly+i]=input->data[i*input->lx+j];
    }    
  }
