...
/*
	$Id: detector.c,v 1.1 2003/09/04 15:44:34 terben Exp $
	$Author: terben $
		...
		double	*	dit)
{
    ...
    /* Allocate 4 images to store the results */
    fitres = cube_new(in->lx, in->ly, 4);
    for (k=0 ; k<4 ; k++) {
    ...
    for (j=0 ; j<in->ly ; j++) {
        ...
        for (i=0 ; i<in->lx ; i++) {
            ...
            if (mx == NULL) {
                fitres->plane[0]->data[pos] = 0 ;
                fitres->plane[1]->data[pos] = 0 ;
                fitres->plane[2]->data[pos] = 0 ;
                fitres->plane[3]->data[pos] = 0 ;
            } else {
                /* Store results in a, b, c images */
                fitres->plane[0]->data[pos] = (pixelvalue)mx->m[0] ;
                fitres->plane[1]->data[pos] = (pixelvalue)mx->m[1] ;
                fitres->plane[2]->data[pos] = (pixelvalue)mx->m[2] ;
                ...
                for (k=0 ; k<in->np ; k++) {
                    ...
                    y = mx->m[0] * ma->m[k] +
                        mx->m[1] * ma->m[k+in->np] +
                        mx->m[2] * ma->m[k+2*in->np] ;
                    /* Error between model and reality */
                    err = y - mb->m[k] ;
                    sq_err += err * err ;
                }
                sq_err /= (double)in->np ;
                fitres->plane[3]->data[pos] = (pixelvalue)sq_err ;
                matrix_del(mx);
            }
            image_del(timeline);
        }
