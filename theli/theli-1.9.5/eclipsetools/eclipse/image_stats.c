...
/*
	$Id: image_stats.c,v 1.3 2005/01/16 19:55:04 terben Exp $
	$Author: terben $
	$Date: 2005/01/16 19:55:04 $
	$Revision: 1.3 $

11.01.2005:
I included a definition of M_PI.

16.01.2005:
I included function 'image_getmedian_vig_lowhighreject' that is
identical to 'image_getmedian_vig' but rejects pixels lower/higher
		...
		int			*	zone)
{
	...
	if (bufsz==1) {
		...
		ret_stats->stdev =
			sqrt((sqsum-((sum*sum)/(double)bufsz))/((double)bufsz-1.0));
	}
...
/**
  ...
  @param	urx	       x pos of upper right corner.
  @param	ury	       y pos of upper right corner.
  @param	lo_rej	       low pixel rejection threshold.
  @param	hi_rej	       high pixel rejection threshold.
  @param	def_median     default return if no median can be estimated.
  @return	1 pixelvalue.
  ...
  the FITS convention (x from 1 to lx and from left to right, y from 1 to
  ly and from bottom to top). Corners are included in the vignette.
  Before the median is estimated, pixels lower and higher than given
  threshholds are rejected. If No pixels are left after rejection, 
  a default value is returned.
 */
/*----------------------------------------------------------------------------*/
pixelvalue image_getmedian_vig_lowhighreject(
		image_t * 	in,
		int			llx,
		int			lly,
		int			urx,
                ...
                pixelvalue              def_median)
{
	...
	median = median_pixelvalue_lowhighreject(clone->data, lo_rej, hi_rej, 
                                                 def_median, clone->lx * clone->ly) ;
	image_del(clone) ;
	return median ;
}
...
double image_getstdev(image_t * image_in)
{
	...
	return sqrt((sqr_sum-((pix_sum*pix_sum)/(double)npix))/((double)npix-1.0));
}
