...
# the scripts creates a check plot for set quantities.  out of the
# chips.cat5 STATS table. It needs the STATS table (chips.cat5)
# including relative photometric zeropoint information.
...
#$6: SuperMongo postscript device (OPTIONAL: defaulted to postportfile)
...
if [ ! -f ${CAT} ]; then
  theli_error "no $3 in /$1/$2/cat/; exiting"
  exit 1
fi 
