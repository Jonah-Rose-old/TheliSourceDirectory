...
do
  if [ ${NOTPROCESS[${CHIP}]:=0} -eq 0 ]; then
    ...
    theli_warn "Chip ${CHIP} will not be processed in $0"  
  fi
done
...
do
  if [ ${NOTPROCESS[${CHIP}]:=0} -eq 0 ]; then
    ...
    if [ -f ${RESULTDIR[${CHIP}]}/$3_${CHIP}.fits ]; then
       theli_error "${RESULTDIR[${CHIP}]}/$3_${CHIP}.fits already present !!! Exiting !!"
       exit 1;
    fi
...
# clean up and bye:
test -f ${TEMPDIR}/flat_images_$$ && rm ${TEMPDIR}/flat_images_$$
