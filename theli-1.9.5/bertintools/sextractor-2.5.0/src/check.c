...
/* 23.05.2013:
   I add THELI HISTORY information to newly created check images
*/
...
/* Inherit the field FITS header */
  ...
  headnblocks=check->fitsheadsize/FBSIZE;
/* keep some margin for HISTORY keywords */
  QMALLOC(check->fitshead, char, (headnblocks + 1) * FBSIZE);
  memset(check->fitshead, ' ', (headnblocks + 1) * FBSIZE);
...
/* add THELI history information */
  fitsadd(check->fitshead, "HISTORY ", "");
  snprintf(tmpstring, 71, "sex_theli: %s", __PIPEVERS__);
  fitsadd(check->fitshead, "HISTORY ", tmpstring);
  snprintf(tmpstring, 71, "sex_theli: called at %s", get_datetime_iso8601());
  fitsadd(check->fitshead, "HISTORY ", tmpstring);

  /* which type of checkimage was created ? */
  switch(check->type)
    {
    case CHECK_IDENTICAL:
	snprintf((char *)tmpstring, 71,
                 "sex_theli: checkimage IDENTICAL from %s", 
                 get_basename(field->filename));
        fitsadd(check->fitshead, "HISTORY ", (char *)tmpstring);
	snprintf((char *)tmpstring, 71,
                 "sex_theli: det thresh.: %f; minarea: %d", 
                 prefs.dthresh[0], prefs.ext_minarea);
        fitsadd(check->fitshead, "HISTORY ", (char *)tmpstring);
	break;
    case CHECK_BACKGROUND:
	snprintf((char *)tmpstring, 71,
                 "sex_theli: checkimage BACKGROUND from %s", 
                 get_basename(field->filename));
        fitsadd(check->fitshead, "HISTORY ", (char *)tmpstring);
	snprintf((char *)tmpstring, 71,
                 "sex_theli: det thresh.: %f; minarea: %d", 
    ...
    case CHECK_FILTERED:
	snprintf((char *)tmpstring, 71,
                 "sex_theli: checkimage FILTERED from %s", 
                 get_basename(field->filename));
        fitsadd(check->fitshead, "HISTORY ", (char *)tmpstring);
	snprintf((char *)tmpstring, 71,
                 "sex_theli: det thresh.: %f; minarea: %d", 
    ...
    case CHECK_OBJECTS:
	snprintf((char *)tmpstring, 71,
                 "sex_theli: checkimage OBJECTS from %s", 
                 get_basename(field->filename));
        fitsadd(check->fitshead, "HISTORY ", (char *)tmpstring);
	snprintf((char *)tmpstring, 71,
                 "sex_theli: det thresh.: %f; minarea: %d", 
  ...
  /* bring header to correct size */
  headnblocks = ((fitsfind(check->fitshead, "END     ") + 36) * 80) / FBSIZE;
  check->fitsheadsize = headnblocks * FBSIZE;
  QREALLOC(check->fitshead, char, headnblocks*FBSIZE);
