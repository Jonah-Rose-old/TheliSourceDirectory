 ...
 * 24.11.2010:
 * routine init_field:
 * The WCS creation code (at least including user settings via -CENTER
 * keywords is only executed if images are resampled. That gave incorrect
 * results if only an image header should be output from swarp - corrected.
 */
...
/* Produce a floating point output */
  ...
  if (prefs.resample_flag || prefs.headeronly_flag)
    {
...
/*-------- This is a special case where CRPIX is not NAXISN/2 */
		...
		wcsmax[n] - wcs->crval[n];
          if ((n==lng || n==lat) && lat!=lng)
            ...
            if (val<0.0)
              val += 360.0;
              wcs->crpix[n] = (double)((int)(fmod(val, 180.0)/pixscale[n])) + 
                1.0;
/*-------- Add a 5% margin in field size */
		...
		= (int)(fmod(wcsmax[n]-wcsmin[n]+360.0, 360.0)*1.05/pixscale[n])
		  + 1;
            ...
            {
              wcs->crpix[n] = (double)((int)(val/pixscale[n])) + 1.0;
              tab->naxisn[n] = wcs->naxisn[n]
