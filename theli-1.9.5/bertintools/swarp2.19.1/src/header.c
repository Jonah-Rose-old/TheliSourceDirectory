...
/*
 * 24.11.2010:
 * function writefitsinfo_outfield: we now add 50 dummy keywords at
 * the end of the output header: this is for adding additional
 * information to the image header after the coaddition process. We
 * also add THELI HISTORY information to output headers
 *
 * 26.11.2010: 
 * - When constructing the output header I suppress several COMMENT
 *   line writings with (!field->headflag) if-statements. This is very
 *   special to the THELI processing and makes our header formats more
 *   nicely (we first create a header only with all necessary information.
 *   The additionally written comments then just 'do not look nice'.
 * - several fixes for swarp derived keywords of saturation level, gain and
 *   exposure time. If only an output header is requested, those keys
 *   were set to default values and later introduced with these values 
 *   into resampled images and further to the co-added image. Several fixed
 *   to get a consistent treatment which writes correctly entries to resampled
 *   and co-added images. We omit these keys of only a header is requested.
 *
 * 28.12.2010:
 * The header keywords for exposure time, gain and saturation level are now
 * written in float instead of exponential representation.
 */
...
void	writefitsinfo_outfield(fieldstruct *field, fieldstruct *infield)
  ...
  if (!prefs.headeronly_flag) {
    ...
    addkeywordto_head(tab, "COMMENT ", "SWarp derived parameters");
    addkeywordto_head(tab, "EXPTIME", 
                      "Maximum equivalent exposure time (s)");
    fitswrite(tab->headbuf, "EXPTIME", &field->exptime, 
              H_FLOAT, T_DOUBLE);
    addkeywordto_head(tab, prefs.gain_keyword, 
                      "Maximum equivalent gain (e-/ADU)");
    fitswrite(tab->headbuf, prefs.gain_keyword, &field->fgain, 
              H_FLOAT, T_DOUBLE);
    addkeywordto_head(tab, prefs.sat_keyword, 
                      "Saturation Level (ADU)");
    fitswrite(tab->headbuf, prefs.sat_keyword, &field->fsaturation, 
              H_FLOAT, T_DOUBLE);
  }
  if (!field->headflag) {
    addkeywordto_head(tab, "COMMENT ", "");
  }
  addkeywordto_head(tab, "SOFTNAME", 
...
/* COMBINE_TYPE */
  ...
  if (infield && (intab=infield->tab) && prefs.ncopy_keywords) {
    ...
    for (k=0; k<prefs.ncopy_keywords; k++) {
      if ((l=fitsfind(intab->headbuf, prefs.copy_keywords[k])) != 
          RETURN_ERROR) {
...
/* add some dummy keywords for later use */  
  if (!field->headflag) {
    addkeywordto_head(tab, "COMMENT ", "");
    addkeywordto_head(tab, "COMMENT ", "DUMMY keywords for later use");
  }
  
  d=0;
  for(n = 0; n < 50; n++)
  {
    sprintf(gstr, "DUMMY%d", n);
    addkeywordto_head(tab, gstr, "DUMMY keyword");
    fitswrite(tab->headbuf, gstr, &d, H_INT, T_LONG);    
  }
  
/* to be consistent with the other THELI software history information */
  addkeywordto_head(tab, "HISTORY ", "");
  sprintf(gstr, "swarp_theli: %s", __PIPEVERS__);
  addkeywordto_head(tab, "HISTORY ", gstr);
  thetime = time(NULL);
  tm = localtime(&thetime);
  sprintf(gstr,"swarp_theli: called at: %04d-%02d-%02dT%02d:%02d:%02d",
                        tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday,
                        tm->tm_hour, tm->tm_min, tm->tm_sec);  
  addkeywordto_head(tab, "HISTORY ", gstr);
...
/* Exposure Time */
  addkeywordto_head(tab, "EXPTIME", "total exposure time(s)");
