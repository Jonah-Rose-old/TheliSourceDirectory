...
/*
	$Id: fits_h.c,v 1.4 2014/02/21 17:19:53 thomas Exp $
	$Author: thomas $
	$Date: 2014/02/21 17:19:53 $
	$Revision: 1.4 $
*/
...
/*
 24.05.2005:
 - function qfits_header_del:
   I fixed a bug when removing the first element. In this case the
   linked list was not updated properly.

 - function keytuple2str:
   If the key is 'XTENSION', a special string format is applied
   to satisfy the FITS standard.

 17.09.2010:
 keytype structure:
 I swapped the values (priorities) of COMMENT and HISTORY keywords.
 I want that HISTORY keywords alwasys are put at the end of
 image headers. Therefore the prioroty of that key has to be
 lower than that of COMMENT.

 21.02.2014:
 keytuple2str function:
 bug fix. A strcpy command copied 'its own string'. This led to a
 core dump under MAC OSX 10.9. I do not really know why strcpy shows
 this behaviour. I admit that overwriting the own string is strange
 though!
 */
/*-----------------------------------------------------------------------------
   								Includes
...
static keytuple *	keytuple_new(char *, char *, char *, char *);
static void 		keytuple_del(keytuple * k);
static void 		keytuple_dmp(keytuple * k);
static keytype 		keytuple_type(char * key);
...
int qfits_header_getint(qfits_header * hdr, char * key, int errval)
{
	...
	if (sscanf(c, "%lg", &d)!=1) return errval ;
	return d ;
}
        ...
        char * com)
{
	...
	/* The old expression strcpy(cval, qfits_pretty_string(cval));
	   led to a coredump under MAC OSX 10.9. I could not figure
	   out why 'overwriting the own string' is not allowed!
	*/
	strcpy(cval_tmp, qfits_pretty_string(cval));
	j=0 ;
        i=0 ;
        while (cval_tmp[i] != (char)0) {
	    if (cval_tmp[i]=='\'') {
	      	cval_q[j]='\'';
	       	j++ ;
	       	cval_q[j]='\'';
        ...
        if (hierarch) {
	       ...
	       if (strncmp(key, "XTENSION", 8) == 0) {
	            sprintf(safe_line, "%-8.8s= '%-8s'           / %s", key, cval_q, ccom);
	       } else {
	            sprintf(safe_line, "%-8.8s= '%-18s' / %s", key, cval_q, ccom);
	       }
	}
	strncpy(line, safe_line, 80);
