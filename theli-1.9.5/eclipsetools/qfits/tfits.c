...
/*
	$Id: tfits.c,v 1.1 2003/09/04 15:44:42 terben Exp $
	$Author: terben $
		...
		int 		xtnum)
{
	...
	if (qfits_get_datinfo(filename, xtnum, &offset_beg, NULL)!=0) {
        ...
        int             *   selection)
{
	...
	/* Load input file */
    if ((start=falloc(th->filename, 0, NULL))==NULL) {
		...
		void			*	null_value)
{
	...
	switch(col->atom_type) {
		...
		out_array = (unsigned char*)qfits_query_column(th, colnum, selection) ;
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)== (int)((unsigned char*)out_array)[i]) {
		...
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)==(int)((short*)out_array)[i]) { 	
		...
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)==((int*)out_array)[i]) { 	
				((int*)out_array)[i] = inull ;
			}
		}
		break ;
		...
		int				*	nb_nulls)
{
	...
	*nb_vals = 0 ;
	...
	switch(col->atom_type) {
		...
		*nb_vals = nb_rows * col->atom_nb ;
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)==(int)((unsigned char*)tmp_array)[i]) {
				out_array[i] = 1 ;
				(*nb_nulls)++ ;
			}
		}
		...
		*nb_vals = nb_rows * col->atom_nb ;
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)==(int)((short*)tmp_array)[i]) { 	
				out_array[i] = 1 ;
				(*nb_nulls)++ ;
			}
		}
		...
		*nb_vals = nb_rows * col->atom_nb ;
		for (i=0 ; i<nb_rows * col->atom_nb ; i++) {
			if (atoi(col->nullval)==((int*)tmp_array)[i]) { 	
				out_array[i] = 1 ;
				(*nb_nulls)++ ;
			}
		}
...
/*----------------------------------------------------------------------------*/
/**
  @brief	Appends an extension header + data to a FITS BIN table file.
        ...
        void            **  data)
{
    ...
    for (i=0 ; i<t->nc ; i++) {
        ...
        if (curr_col->zero_present) {
            ...
            qfits_header_append(fh, str_val, str_val2, "NULL value is defined",
                    NULL) ;
        }
        ...
        if (curr_col->scale_present) {
            ...
            qfits_header_append(fh, str_val, str_val2, "Scaling applied", NULL);
        }

        curr_col++ ;
    }
	
	qfits_header_append(fh,"ORIGIN","ESO-ECLIPSE","Written by ECLIPSE", NULL);
    ...
    for (i=0 ; i<t->nc ; i++) {
        ...
        array[i] = malloc(t->nr * curr_col->atom_size * curr_col->atom_nb) ;
        r = (unsigned char *)array[i] ;
        inbuf = (unsigned char *)(data[i]) ;

        for (j=0 ; j<t->nr ; j++) {
            memcpy(r, inbuf,
                    curr_col->atom_nb*curr_col->atom_size) ;
            inbuf += (curr_col->atom_nb * curr_col->atom_size) ;
            r += (curr_col->atom_nb * curr_col->atom_size) ;
        }
    ...
    for (i=0 ; i<t->nr ; i++) {
        curr_col = t->col ;
        for (j=0 ; j<t->nc ; j++) {
            r = array[j] + curr_col->atom_size*curr_col->atom_nb*i ;
            memcpy(line, r, curr_col->atom_size*curr_col->atom_nb) ;
            line[curr_col->atom_size*curr_col->atom_nb] = (char)0 ;
            fwrite(line, 1, curr_col->atom_size*curr_col->atom_nb, outfile) ;
            writt_char += curr_col->atom_size*curr_col->atom_nb ;
            curr_col++ ;
        }
    }
        ...
        void            **  data)
{
    ...
    for (i=0 ; i<t->nc ; i++) {
        ...
        qfits_header_append(fh, str_val, str_val2,"Start column of field",NULL);
    ...
    for (i=0 ; i<t->nc ; i++) {
		/* Copy data from data to array (unsigned char) */
        array[i] = malloc(t->nr * curr_col->atom_nb) ;
        r = (unsigned char *)array[i] ;
        inbuf = (char *)(data[i]) ;
        for (j=0 ; j<t->nr ; j++) {
			switch(curr_col->atom_type) {
				case TFITS_ASCII_TYPE_A :
					strncpy(field, inbuf, curr_col->atom_nb) ;
    ...
    for (i=0 ; i<t->nr ; i++) {
        curr_col = t->col ;
        for (j=0 ; j<t->nc ; j++) {
            r = array[j] + curr_col->atom_nb*i ;
            memcpy(line, r, curr_col->atom_nb) ;
            line[curr_col->atom_nb] = (char)0 ;
            fwrite(line, 1, curr_col->atom_nb, outfile) ;
            writt_char += curr_col->atom_nb ;
            curr_col++ ;
        }
    }
