...
/**
  ...
  capabitilies to ANSI C programs running on POSIX-compliant systems. If 
...
/*
    $Id: xmemory.c,v 1.3 2010/03/26 15:25:59 thomas Exp $
    $Author: thomas $
    $Date: 2010/03/26 15:25:59 $
    $Revision: 1.3 $
*/
...
/* 24.01.2005:
   - I replaced '-1' by MAP_FAILED in chekcs whether mmap calls
     were successful.
   ...
   16.02.2010:
   I included a mode_t argument in calling a C open function.
   Compilation errors were reported when omitting this 
   'optional' parameter.

   26.03.2010:
   I increased the number of maximum pointers that can be
   administrated by xmemory (8192 -> 32771); request from Mischa
*/
...
static int    xmemory_remcell(void *, int);
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Signal handler for this module.
  @param    sig     Signal ID.
  @return   void

  This function handles Unix signals. SIGSEGV and SIGBUS will print
  out a bug message and exit the current process. SIGINT and SIGTERM will
  print a "user interruption" message and exit the current process. SIGFPE
  is caught and ignored. All other signals are ignored.

  The exit code returned by this function is -signal.
 */
/*----------------------------------------------------------------------------*/
static void xmemory_sig(int sig)
{
    switch (sig) {

        case SIGSEGV:
        case SIGBUS:
        /* Segmentation fault: print bug art and exit */
        fprintf(stderr,
                "\n\n"
        ...
        case SIGTERM:
        case SIGINT:
        /* CTRL-C or interrupt signal: abort with message */
        fprintf(stderr, "**** user interrupted process %ld ****\n",
                (long)getpid());
        break;

        default:
        /* Do not handle other signals */
        return ;
    }
    exit(-sig);
}
...
/**
  ...
  This function is implicitly called by the first malloc() or calloc()
  or strdup() execution. It allocates a minimal number of memory cells into
  the global extended memory table. It also installs signal handlers and
  atexit routines the first time it is called, and increases the number of
  possible descriptors to the maximum.
 */
/*----------------------------------------------------------------------------*/
static void xmemory_init(void)
{
    ...
    /* Signal and atexit initialization */
    xmem_debug(
        fprintf(stderr, "xmem: initializing signal catcher and atexit\n");
    );

    /* Catch signals */
    signal(SIGTERM, xmemory_sig);
    signal(SIGINT , xmemory_sig);
    signal(SIGSEGV, xmemory_sig);
    signal(SIGBUS , xmemory_sig);
    signal(SIGFPE , xmemory_sig);
    ...
    xmemory_table.rlimit_data = (size_t)rlim.rlim_cur ;
    xmem_debug(
...
/*----------------------------------------------------------------------------*/
/**
  @brief    Add allocation cell to xtended memory table.
  ...
  allocation took place. This call is not protected against illegal
  parameter values, so make sure the passed values are correct!

  This function returns the index of the memory cell in the global
  table.
 */
/*----------------------------------------------------------------------------*/
    ...
    char    *   mm_filename)
{
    int i ;
    /*
     * This call is not protected against invalid values in input, since it
     * is private to this module and only called in one place.
     */
    ...
    /* Store information */
    xmemory_table.cell[i].pointer       = pointer ;
    xmemory_table.cell[i].size          = size ;

#if (XMEMORY_DEBUG>=1)
    if (filename) {
        strncpy(xmemory_table.cell[i].filename,
                filename,
                SRCFILENAMESZ-1);
    } else {
        xmemory_table.cell[i].filename[0] = (char)0 ;
    }
    xmemory_table.cell[i].lineno        = lineno ;
#endif

    xmemory_table.cell[i].memtype       = memtype ;
    xmemory_table.cell[i].swapfileid    = swapfileid ;
    xmemory_table.cell[i].swapfd        = swapfd ;

    if (mm_filename!=NULL) {
        strncpy(xmemory_table.cell[i].mm_filename, mm_filename, MAPFILENAMESZ);
        xmemory_table.cell[i].mm_hash = xmemory_hash(mm_filename);
        xmemory_table.cell[i].mm_refcount = 1 ;
    } else {
        xmemory_table.cell[i].mm_filename[0] = 0 ;
        xmemory_table.cell[i].mm_hash = 0 ;
        xmemory_table.cell[i].mm_refcount = 0 ;
    }
...
/**
  ...
  @param    ptr     Pointer just deallocated.
  @param    pos     Position of the pointer in the table.
  @return   int 0 if Ok, -1 if error occurred.

  This function takes in input the name of a pointer supposedly just
  deallocated, and a possible position in the table. If the provided
  position is -1, a search is made on the table to locate the pointer,
  otherwise the position is trusted and used to deallocate the table entry.

  Once the pointer is found, all references to it are deleted from the
  table.

  If the pointer cannot be found in the table, it means that a deallocation
  has been requested on an invalid pointer. -1 is returned in that case. If
  no error occurs, this function returns 0.

  Deallocating might take some time, if the memory table has to shrink by a
  factor 2. This should only happen when a power-of-2 boundary is reached
  for the number of cells in the table.
 */
/*----------------------------------------------------------------------------*/
static int xmemory_remcell(void * ptr, int pos)
{
    int i ;

    /* Trust input pointer position or locate it */
    if (pos>=0) {
        i = pos ;
    } else {
        /* Look for a matching pointer */
        for (i=0 ; i<XMEMORY_MAXPTRS ; i++) {
            if (xmemory_table.cell[i].pointer == ptr) {
                break ;
            }
        }
        if (i>=XMEMORY_MAXPTRS) {
            xmem_debug(
                fprintf(stderr, "xmem: xmemory_remcell: cannot locate pointer");
            ) ;
            return -1 ;
        }
    }
...
static void xmemory_dumpcell(int pos, FILE * out)
{
    ...
    if (xmemory_table.cell[pos].pointer==NULL) return ;

    if (xmemory_table.cell[pos].memtype == MEMTYPE_MMAP) {
#if (XMEMORY_DEBUG>=1)
        fprintf(out,
            "M(%p) - %s (%d) maps [%s] for %ld bytes",
            xmemory_table.cell[pos].pointer,
...
/**
  ...
  Notice that if the @c XMEMORY_DIEONNOALLOC symbol is defined to 1, this
  function might exit the program (if the system's malloc returns NULL).
  Change the value of the symbol to turn that behaviour off, but you need
  to check the return values of your malloc calls in that case.
 */
/*----------------------------------------------------------------------------*/
void * xmemory_malloc(size_t size, char * filename, int lineno)
{
    ...
    if (ptr==NULL) {
        ...
        xmem_debug(
            fprintf(stderr,
                    "xmem: swap [%s] created for %ld bytes\n",
                    fname, (long)size);
        );
        ...
        for (p=0 ; p<size ; p+=xmemory_table.pagesize) {
            ((char*)ptr)[p] = 0 ;
        }
#endif
        swapfd = -1 ;
...
/**
  ...
  Notice that if the @c XMEMORY_DIEONNOALLOC symbol is defined to 1, this
  function might exit the program (if the system's calloc returns NULL).
  Change the value of the symbol to turn that behaviour off, but you need
  to check the return values of your calloc calls in that case.
 */
/*----------------------------------------------------------------------------*/
...
/**
  ...
  a char pointer. It also modifies the input size variable to be
        ...
        int         srclin)
{
    ...
    if (xmemory_table.ncells>0) {
        ...
        for (i=0 ; i<XMEMORY_MAXPTRS ; i++) {
            ...
            if ((xmemory_table.cell[i].pointer!=NULL) &&
                ...
                if (!strncmp(xmemory_table.cell[i].mm_filename,
                             name,
                             MAPFILENAMESZ)) {
                    /* File already mapped */
                    ...
                    ptr = (char*)xmemory_table.cell[i].pointer + offs ;
    ...
    /* Close file */
    close(fd);
    if (ptr == (char*)MAP_FAILED || ptr==NULL) {
        xmem_debug(
            perror("mmap");
...
/**
  ...
  Free the memory associated to a given pointer. Prints out a warning
  on stderr if the requested pointer is NULL or cannot be found in the
...
void xmemory_free(void * ptr, char * filename, int lineno)
{
    ...
    for (i=0 ; i<XMEMORY_MAXPTRS ; i++) {
        ...
        if (xmemory_table.cell[i].memtype==MEMTYPE_MMAP) {
            if (((char*)xmemory_table.cell[i].pointer<=(char*)ptr) &&
                (((char*)xmemory_table.cell[i].pointer + 
    ...
    switch (xmemory_table.cell[pos].memtype) {
        ...
        munmap((char*)xmemory_table.cell[pos].pointer,
