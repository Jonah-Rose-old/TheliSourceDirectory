 ...
 * 07.03.2007:
 * function readprefs:
 * the function has a new argument giving the maximum stringlength
...
documentation)
*/
void    readprefs(char *filename, char **argkey, char **argval, 
                  int narg, size_t maxargstringlength)

  {
   FILE          *infile;
   char          *cp, *str, *keyword, *value, **dp;
   size_t        strlength;  /* maximum length of str string */
  ...
  if(maxargstringlength > MAXCHAR)
  {
      strlength = maxargstringlength;
  }
  else
  {
      strlength = MAXCHAR;
  ...
  for (warn=0;;)
    ...
    if (flagd)
      ...
      if (**dp)
        {
        if (**dp=='*')
          strncpy(str, *(dp++)+1, strlength);
        else
          strncpy(str, *(dp++), strlength);
        }
      else
        flagd = 0;
      }
    ...
    if (!flagc && !flagd)
    {
      if (flage || !fgets(str, strlength, infile))
      {
        flagc=1;
      }
    }
    ...
    if (flagc)
      ...
      if (argi<narg)
      { 
        snprintf(str, strlength, "%s %s", argkey[argi], argval[argi]);
        argi++;
      }
      else
        break;
