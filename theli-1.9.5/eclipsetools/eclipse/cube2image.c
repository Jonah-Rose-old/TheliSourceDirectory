...
/*
	$Id: cube2image.c,v 1.3 2010/03/31 12:00:52 thomas Exp $
	$Author: thomas $
	$Date: 2010/03/31 12:00:52 $
	$Revision: 1.3 $
*/

/*
  25.11.2004:
  corrected a bug in cube_avg_median_minmax_rej:
  the median was only estimated if more than 3
  images could be stacked. It is sufficient and
  it was intended to do this already with 3
  images.

  31.03.2010:
  function cube_avg_median_minmax_rej:
  I lowered the huumber of necessary pixels for a median estimate
  from 3 to 2 (request from Mischa)
*/
...
/**
  @brief    Average a cube to a median image 
            rejecting pixels below and above a given value.
	    From the remaining pixels the lo_rej lowest
	    and hi_rej highest values are also not used
	    in the final median. If less than 2 pixels remain
	    at any stage a default value is returned. 
  @param    to_average  Cube to average.
  @param    min       minimum threshhold to accept pixels
  @param    max       minimum threshhold to accept pixels
  @param    lo_rej    Number of min pixels to reject.
  @param    hi_rej    Number of max pixels to reject.
  @param    def       Default value to return for less than 5 images in median
  @return	Newly allocated image object.

  The returned image is a median image of the input cube. See the
  convention for median used in the case of an even number of
  elements, in median.c
 */
/*--------------------------------------------------------------------------*/
image_t * cube_avg_median_minmax_rej(cube_t * to_average, /* input cube */ 
                                     pixelvalue min,      /* minimum value to 
                                                             accept pixels */
				     pixelvalue max,      /* maximum value to 
                                                             accept pixels */
		                     int  lo_rej,         /* the number of low 
                                                             values to reject */
		                     int  hi_rej,         /* the number of high 
                                                             values to reject */
                                     pixelvalue def)      /* default value if less 
                                                             than 3 pix are left */
{
	image_t	*	avg ;
	int				plane, nplanes;
	...
	for (j=0 ; j<to_average->ly ; j++) {
	     ...
	     for (i=0 ; i<to_average->lx ; i++) {
		  ...
		  accepted = 0;
		  for (plane=0 ; plane<nplanes ; plane++) {
		      if((to_average->plane[plane]->data[pos] > min) &&
                         (to_average->plane[plane]->data[pos] < max))
		      {
		         timeline[accepted] = to_average->plane[plane]->data[pos] ;
		         accepted ++;
		      }
		  }

		  if((accepted-lo_rej-hi_rej) >= 2)
		  {
	            pixel_qsort(timeline, accepted) ;
		    avg->data[pos] = median_pixelvalue(&timeline[lo_rej], 
						       accepted-lo_rej-hi_rej) ;
		  }
		  else
		  {
		    avg->data[pos] = def ;
