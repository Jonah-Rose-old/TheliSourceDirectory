...
do
  ...
  FILTER=`get_key ${FILE} "HIERARCH ESO INS OPTI6 NAME" | ${P_CLEANSTRING}`
...
# extract only the first fits extension for NACOSDI@VLT
...
# cut the four image sections

  mv ${BASE}_1.fits ${BASE}_tmp.fits

  ${P_FITSCUT} -i ${BASE}_tmp.fits -o ${BASE}_1.fits -x 68 329 677 933
  ${P_FITSCUT} -i ${BASE}_tmp.fits -o ${BASE}_2.fits -x 564 819 488 715
  ${P_FITSCUT} -i ${BASE}_tmp.fits -o ${BASE}_3.fits -x 261 517 177 410
  ${P_FITSCUT} -i ${BASE}_tmp.fits -o ${BASE}_4.fits -x 750 1006 1 212
