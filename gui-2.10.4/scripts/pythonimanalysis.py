...
order=sys.argv[3]
pixscale=float(sys.argv[4])
xmax=float(sys.argv[5])
ymax=float(sys.argv[6])
colorrange=float(sys.argv[7])/100.
units=sys.argv[8]
method=sys.argv[9]
scalemin=sys.argv[10]
scalemax=sys.argv[11]
scaling=sys.argv[12]
...
# derived quantities
ellp = sqrt(e1*e1 + e2*e2)
epsx = ellp * cos(theta*3.14159/180.)
epsy = ellp * sin(theta*3.14159/180.)
...
# polynomial
def peval(x,y,p,order):
    if order == 1:
        return p[0]+p[1]*x+p[2]*y
    if order == 2:
        return p[0]+p[1]*x+p[2]*y+p[3]*x**2+p[4]*y**2
    if order == 3:
        return p[0]+p[1]*x+p[2]*y+p[3]*x**2+p[4]*y**2+p[5]*x*x*x+p[6]*y*y*y

# residuals between data and polynomial
def residuals(p, z, x, y):
    return z - peval(x,y,p,order)
...
if order == 3:
    param_start = [0.,0.,0.,0.,0.,0.,0.]

if method == "Polynomial":
    param_fwhm_final = leastsq(residuals, param_start, args=(fwhm,xpos,ypos))
    #param_ellx_final = leastsq(residuals, param_start, args=(epsx,xpos,ypos))
    #param_elly_final = leastsq(residuals, param_start, args=(epsy,xpos,ypos))
    fwhm_fit = peval(xgrid,ygrid,param_fwhm_final[0],order)
    #ellx_fit = peval(xpos,ypos,param_ellx_final[0],order)
    #elly_fit = peval(xpos,ypos,param_elly_final[0],order)

if method == "Spline":
    eps = sqrt(xmax*ymax/nobj)
    rbf = Rbf(xpos, ypos, fwhm, smooth=eps/10.)
...
#   xp1[i] = xpos[i] - 0.5 * ellx_fit[i] * stickscale
#   yp1[i] = ypos[i] - 0.5 * elly_fit[i] * stickscale
#   xp2[i] = xpos[i] + 0.5 * ellx_fit[i] * stickscale
#   yp2[i] = ypos[i] + 0.5 * elly_fit[i] * stickscale
...
fig.text(0.5,0.05,"("+str(nobj)+" stars used)")
