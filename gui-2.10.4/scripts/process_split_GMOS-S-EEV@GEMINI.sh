...
do
  ...
  if [ "${MJD}" = "" ]; then
      MJD=`${P_DFITS} -x 1 ${FILE} | ${P_FITSORT} MJD-OBS | \
	  ${P_GAWK} '(NR==2) {if ($2 !="KEY_N/A") print $2}'`
      if [ "${MJD}"_A = "_A" ]; then
	  MJD=52000.0
      fi
  fi
  ...
  do
    ...
    ${P_REPLACEKEY} ${BASE}_${k}.fits "DATE-OBS= ${tmp1}" DUMMY1
    tmp2=`get_key ${FILE} "TIME-OBS"`
    ${P_REPLACEKEY} ${BASE}_${k}.fits "TIME-OBS= ${tmp2}" DUMMY2
    ${P_REPLACEKEY} ${BASE}_${k}.fits "MJD-OBS = ${MJD}" DUMMY3
    ${P_REPLACEKEY} ${BASE}_${k}.fits "ORIGFILE= ${FILE}" DUMMY4
    mv ${BASE}_${k}.fits GMOSS.${FILTER}.${tmp1}T${tmp2}_${k}.fits
    k=$(( $k + 1 ))
  done
