...
# deal with the funny aperture stuff of the WHT system:
  ...
  ROTSKYPA=`get_key ${FILE} "ROTSKYPA" | awk '{print (-1)*$1}'`

  ${P_ROTATECDMATRIX} -c ${PIXSCX} 0.0 0.0 ${PIXSCY} -p ${ROTSKYPA} > ${TEMPDIR}/lirishead_$$
  cd11=`awk '(NR==1) {print $3}' ${TEMPDIR}/lirishead_$$`
  cd12=`awk '(NR==2) {print $3}' ${TEMPDIR}/lirishead_$$`
  cd21=`awk '(NR==3) {print $3}' ${TEMPDIR}/lirishead_$$`
  cd22=`awk '(NR==4) {print $3}' ${TEMPDIR}/lirishead_$$`
  rm ${TEMPDIR}/lirishead_$$
  ...
  ${P_REPLACEKEY} ${BASE}_1.fits "DATE-OBS= ${DATEOBS}" DUMMY3
